...
import club.nsdn.nyasamaelectricity.util.catenary.Catenary;
 ...
 */
public class CatenaryRenderer extends AbsFastTESR {

    final AbsFastTESR wireRenderer;
    ...
    public void renderTileEntityFast(
        ...
        if (te.getBlockType() == BlockLoader.catenaryNode) {
            wireRenderer.renderTileEntityFast(te, x, y, z, partialTicks, destroyStage, partial, buffer);
            return;
        }
        ...
        if (te instanceof WireNode.TileEntityWireNode) {
            ...
            if (srcVec != null) {
                if (wireNode.srcQuads.isEmpty()) {
                    RawQuadGroup group = Catenary.renderCatenaryCable(srcVec, theVec, texture);
            ...
            if (dstVec != null) {
                if (wireNode.dstQuads.isEmpty()) {
                    RawQuadGroup group = Catenary.renderCatenaryCable(theVec, dstVec, texture);
